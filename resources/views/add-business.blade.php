...
<body class="flex flex-col min-h-screen text-slate-800 font-sans antialiased">
    ...
    <main class="flex-grow max-w-3xl w-full mx-auto px-4 py-12">
        <div class="bg-white p-8 rounded-2xl shadow-xs border border-slate-100">
            ...
            @if(session('success'))
                <div class="bg-emerald-50 border border-emerald-200 text-emerald-700 p-4 rounded-xl text-xs font-bold mb-6">
                    {{ session('success') }}
                </div>
            @endif
            ...
            <form action="{{ route('business.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-700 mb-1.5">المحافظة الرئيسية *</label>
                        <select id="governorate-select" required class="w-full text-xs p-3 border border-slate-200 rounded-xl bg-slate-50 focus:outline-emerald-500 font-bold">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-700 mb-1.5">المنطقة / المدينة الفرعية *</label>
                        <select id="region-select" name="location_id" required class="w-full text-xs p-3 border border-slate-200 rounded-xl bg-slate-200 cursor-not-allowed font-bold" disabled>
                ...
                <button type="submit" class="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs py-3.5 rounded-xl shadow-xs transition-colors cursor-pointer">
                    🚀 إرسال المنشأة للمراجعة والنشر الآن
                </button>
            </form>
        </div>
    </main>
    ...
    <footer class="bg-slate-900 text-slate-400 text-xs py-6 text-center border-t border-slate-800">
        <p>© {{ date('Y') }} دليل سوريا التجاري. جميع الحقوق محفوظة.</p>
    </footer>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // ✅ النسخة النهائية المبسطة -直接用 الرابط المطلق
            function fetchRegions(governorateId) {
                // عرض رسالة تحميل
                regionSelect.innerHTML = '<option value="">⏳ جاري تحميل المناطق...</option>';
                regionSelect.disabled = true;
                regionSelect.classList.add('bg-slate-200');
                regionSelect.classList.remove('bg-slate-50');
                ...
                // ✅ استخدام الرابط المطلق الكامل (https)
                const apiUrl = `https://aza-international.com/dlil/get-regions/${governorateId}`;
                console.log('🌐 جاري الاتصال بـ:', apiUrl);
                
                // استخدام fetch مع الرابط المطلق
                fetch(apiUrl, {
                    method: 'GET',
                    headers: {
                ...
                .then(data => {
                    ...
                    if (data.length === 0) {
                        regionSelect.innerHTML = '<option value="">⚠️ لا توجد مناطق تابعة لهذه المحافظة</option>';
                        regionSelect.disabled = true;
                    } else {
                        ...
                        regionSelect.classList.remove('bg-slate-200');
                        regionSelect.classList.add('bg-slate-50');
                    }
                })
                .catch(error => {
                    console.error('❌ خطأ في جلب المناطق:', error);
                    regionSelect.innerHTML = `<option value="">❌ خطأ: ${error.message}</option>`;
                    regionSelect.disabled = true;
                    
                    // عرض رسالة خطأ للمستخدم
                    const errorDiv = document.createElement('div');
                    errorDiv.className = 'bg-red-50 border border-red-200 text-red-700 p-3 rounded-xl text-xs mt-2';
                    errorDiv.innerHTML = `⚠️ فشل تحميل المناطق. يرجى تحديث الصفحة والمحاولة مرة أخرى.<br><span class="text-[10px]">${error.message}</span>`;
                    
                    const parentDiv = regionSelect.parentElement;
                    const oldError = parentDiv.querySelector('.error-message-regions');
                    if (oldError) oldError.remove();
                    errorDiv.classList.add('error-message-regions');
            ...
            govSelect.addEventListener('change', function () {
                ...
                console.log('🏙️ تم اختيار المحافظة ID:', govId);
                if (govId) {
                    fetchRegions(govId);
                } else {
                    regionSelect.innerHTML = '<option value="">🏙️ اختر المحافظة أولاً</option>';
                    regionSelect.disabled = true;
                    regionSelect.classList.add('bg-slate-200');
                }
            });
